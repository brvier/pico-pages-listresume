...
class Pico_Pages_ListResume
{
	...
	{
		$twig_vars['pages_listresume'] = $this->output($this->pages);
	}
	...
	private function output($pages)
	{
		...
		{
			//if (!array_key_exists('hide', $page)) continue;
			
			if (!array_key_exists('path', $page)) continue;
			...
			$html .= '<li class="'.$class.'">' . $item . '<br /><small>'.$page['excerpt'].'</small>' . $childs . '</li>';
		}
		$html .= '</ul>';
		return $html;
	}
